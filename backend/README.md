...
2. Environment: copy `.env.example` to `.env` (or edit the local `.env`) and fill in values:

   - `PORT` — server port (default `4000`)
   - `MONGODB_URI` — MongoDB connection string
   - `JWT_SECRET` — secret for JWT signing
   - `CLOUDINARY_*` — Cloudinary credentials (only needed for base64 photo uploads)

   `Endpoints-Collection.json` is a Postman v2.1 import with all GraphQL operations; set the collection variable `employeeId` after you create or list employees.
